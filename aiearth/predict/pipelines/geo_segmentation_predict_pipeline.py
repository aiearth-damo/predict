...
from aiearth.predict.checkpoint import ModelConfig
...
class GeoSegmentationPredictPipeline(Pipeline):
    def __init__(
        self,
        model_checkpoint,
        predictor_cls,
        predictor_num_gpus=1,
        predictor_memory=GiB(1),
        ...
        self.predictor_num_gpus = predictor_num_gpus
        self.predictor_batch_size = predictor_batch_size
        self.predictor_memory = predictor_memory
    ...
    def __call__(
        ...
        try:
            ...
            for window, index in zip(windows, indexes):
                ...
                ckpt_cfg: ModelConfig = self.model_checkpoint.get_cfg()

                if aoi_shape_file:
                    sfvd = ShapeFileVectorDataset(aoi_shape_file)
                    data = read_raster_dataset(
                        raster_dataset,
                        ckpt_cfg.params.image_size,
                        ckpt_cfg.params.bound,
                ...
                if self.pre_processors:
                    pre_processor = Chain(*self.pre_processors)
                    data = pre_processor.transform(data)

                predictor = BatchPredictor.from_checkpoint(
                    self.model_checkpoint, self.predictor_cls
                )

                ray_remote_args = {}
                ray_remote_args["memory"] = self.predictor_memory
                data = predictor.predict(
                    data,
                    feature_columns=["image"],
                    batch_size=self.predictor_batch_size,
                    keep_columns=["box", "pad_bounds"],
                    num_gpus_per_worker=self.predictor_num_gpus,
                    ray_remote_args=ray_remote_args,
                )

                if self.post_processors:
                    post_processor = Chain(*self.post_processors)
